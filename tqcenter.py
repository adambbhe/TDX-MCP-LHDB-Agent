...
class tq:
    ...
    def get_market_data(cls,
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    print(f"[WARN] get_market_data 初始化失败(尝试{attempt+1}/{max_retries}): {e}")
                    time.sleep(0.5)
                    continue
                else:
                    print(f"[ERROR] get_market_data 初始化最终失败: {e}")
                    return {'error': -1, 'msg': f'连接初始化失败: {str(e)[:100]}'}
        # stimeD = time.time()

        # 快速参数验证 (修复: 避免调用close()导致连锁反应)
        if not stock_list:
        ...
        # 修复: 添加向后兼容的格式转换
        # 当前格式: {字段名: DataFrame(列=股票列表)}
        # 兼容格式: {股票代码: DataFrame(行=日期, 列=字段)}
        if result_data and len(stock_list) > 0:
            first_key = list(result_data.keys())[0] if result_data else None
            if first_key in ['Volume', 'Open', 'High', 'Low', 'Close', 'Amount',
                           'ForwardFactor', 'AdjFactor', 'OpenInt', 'HighInt',
                           'LowInt', 'CloseInt', 'VolumeInt', 'AmountInt']:
                # 检测到新格式，转换为兼容格式
                compatible_result = {}
                for stock in stock_list:
                    stock_df_dict = {}
                    ...
                    for field_name, field_df in result_data.items():
                        if stock in field_df.columns:
                            stock_df_dict[field_name.lower()] = field_df[stock]
                            has_data = True

                    if has_data:
                        compatible_result[stock] = pd.DataFrame(stock_df_dict)

                if compatible_result:
                    result_data = compatible_result
        ...
        for stock in stock_list:
            try:
                ...
                if 'Data' not in data_dict or data_dict['Data'] is None:
                    print(f"获取{stock}财务数据: Data字段为空或不存在")
                    continue

                # 修复: 过滤None值，避免len(None)异常
                valid_data = {k: v for k, v in data_dict['Data'].items() if v is not None}
                if not valid_data:
                    print(f"获取{stock}财务数据: 所有字段值均为空")
                    continue

                # 获取所有列表的长度，检查是否一致
                list_lengths = [len(v) for v in valid_data.values()]
    ...
    def order_stock(cls,
        ...
        try:
            ...
            # 修复:将price转换为ctypes.c_double类型以匹配DLL接口要求
            price_ctypes = ctypes.c_double(price)

            timeout_ms = 5000
            ptr = dll.SetNewOrder(cls._get_run_id(), account_str, code, order_type, order_volume,
                                price_type, price_ctypes, remark, timeout_ms)

            # 增强返回值处理:支持多种返回格式
            if ptr is None:
                print(f"下单{stock_code}: DLL返回空指针")
                return -1

            # 情况1: 返回的是整数(错误码)
            if isinstance(ptr, int):
                print(f"下单{stock_code}: DLL返回错误码 {ptr}")
                return {'ErrorId': str(ptr), 'Error': f'DLL返回错误码{ptr}'}

            # 情况2: 返回的是字符串/字节
            try:
                if len(ptr) > 0:
                    result_str = ptr.decode('utf-8') if isinstance(ptr, bytes) else str(ptr)
                    print(f"下单{stock_code}: DLL返回数据 - {result_str[:200]}")  # 打印前200字符用于调试
                    ...
                    try:
                        ...
                        if isinstance(data_json, dict):
                            if data_json.get("ErrorId") != "0":
                                print(f"下单{stock_code}业务错误: {data_json.get('Error', '未知错误')}")
                                return data_json
                            return data_json
                        else:
                            # JSON解析成功但不是字典
                            print(f"下单{stock_code}: JSON非字典格式 - {type(data_json)}")
                            return {'ErrorId': '-1', 'Error': f'返回数据格式异常: {str(data_json)[:100]}', 'RawData': result_str}
                    except json.JSONDecodeError:
                        # 不是JSON格式,可能是纯文本或数字
                        print(f"下单{stock_code}: 非JSON格式返回 - {result_str}")
                        return {'ErrorId': '-1', 'Error': f'返回格式异常', 'RawData': result_str}
                else:
                    print(f"下单{stock_code}: 返回空字符串")
                    return -1
            except Exception as e:
                print(f"下单{stock_code}数据处理异常: {e}")
                return -1
        except Exception as e:
